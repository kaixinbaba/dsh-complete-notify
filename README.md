...
1. 任务完成后：听到「叮咚」提示音 + 右上角弹出「✓ 任务完成」小卡片，包含**一句话小结**（💬 recap，≤50 字）、**运行统计**（时长 ⏱ / tokens ⚡ / 步骤 🔧）与「点击打开会话」提示；点击卡片快速跳转到对应会话，5 秒自动消失
2. 页面切到后台/其他标签页时完成任务：收到**系统通知**（正文同样带小结与统计，点击通知会聚焦窗口并打开对应会话）+ 提示音 + 标签页标题闪烁
3. **设置 → 任务完成通知**：
...
| 场景 | 行为 |
|---|---|
| 页面可见，任一会话完成 | toast（小结 + 时长/token/steps）+ 音效 |
| 页面在后台，会话完成 | 系统通知（含小结与统计）+ 音效 + 标题闪烁 |

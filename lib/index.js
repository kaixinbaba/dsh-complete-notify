...
/** sessionId -> 最近一次 turn/end 的原因（completed/blocked/error/aborted/max-tokens） */
const kinds = new Map()

/** 从 turn/end 会话事件提取结果状态；非 turn/end 返回 null。 */
function kindOfEvent(event) {
  if (!event || event.type !== 'turn/end') return null
  const data = event.data
  const kind = data && data.reason && typeof data.reason.kind === 'string' ? data.reason.kind : 'unknown'
  return kind
}
...
export function apply(ctx) {
  // 0) 记录每个根会话最近一次 turn/end 的结果状态（客户端 toast/通知按状态着色）
  ctx.on('session/event', (session, event) => {
    const kind = kindOfEvent(event)
    if (kind === null) return
    if (session && session.header && session.header.origin === 'subagent') return
    const sessionId = session && session.header && session.header.id
    if (!sessionId) return
    kinds.set(sessionId, kind)
    if (kinds.size > RECAP_CACHE_MAX) kinds.delete(kinds.keys().next().value)
  })
  ...
  // 2) 客户端拉取小结/状态的路由（webServer 服务需等注入就绪后才可用）
    ...
    hostCtx.effect(() => hostCtx.webServer.register({
      ...
      handler: async (request, response) => {
        ...
        try {
          ...
          const kind = kinds.get(sessionId) || null
          sendJson(response, 200, { ok: true, recap, kind })
        } catch (err) {
...
// 单测钩子（宿主加载器会忽略额外导出）
export const __test = { cleanRecap, lastAnswerText, kindOfEvent }
